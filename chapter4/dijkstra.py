...
def dijkstra(wg: WeightedGraph[V], root: V) -> Tuple[List[Optional[float]], Dict[int, WeightedEdge]]:
    ...
    # all distances are unknown
    distances: List[Optional[float]] = [None] * wg.vertex_count
    distances[first] = 0

    # how to find each vertex
    path_dict: Dict[int, WeightedEdge] = {}
    pq: PriorityQueue[DijkstraNode] = PriorityQueue()
    pq.push(DijkstraNode(first, 0))

    while not pq.empty:
        # search next closest node
        u: int = pq.pop().vertex
        ...
        for we in wg.edges_for_index(u):
            ...
            # No older distance found or we found a shortest distance
...
def path_dict_to_path(start: int, end: int, path_dict: Dict[int, WeightedGraph]) -> WeightedGraph:
    if len(path_dict) == 0:
        return []

    edge_path: WeightedGraph = []
    e: WeightedGraph = path_dict[end]
    edge_path.append(e)
